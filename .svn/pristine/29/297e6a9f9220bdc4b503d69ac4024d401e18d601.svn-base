...
<html>
	<head>
        ...
        <script language="javascript" src="js/publicMessageManage.js"></script>
	</head>
...
function Oper(){
    ...
    this.initPublicGrid = function(){
		...
		var column = new Ext.grid.ColumnModel([
		    {header:'主键',dataIndex:'messageId',hidden:true },	
		    {header:'公告发布人id',dataIndex:'createStaffId',hidden:true},
		    {header:'公告发布对象ID',dataIndex:'publicObject',hidden:true },			    
		    {header:'公告标题',dataIndex:'title', width:swidth*0.2},
		    {header:'公告内容',dataIndex:'content',width:swidth*0.25 },
		    {header:'公告时间',dataIndex:'createTime',width:swidth*0.1},		    		    
		    {header:'公告发布人',dataIndex:'createStaffName',width:swidth*0.05},
		    {header:'发布类型',dataIndex:'publicType',width:swidth*0.05,renderer:function (val){ if (val == null || val ==""){return val } else if (val == 1) {return "人员"} else if (val == 2) {return "部门"}}},
		    {header:'公告发布对象',dataIndex:'publicObjectName',width:swidth*0.3}
	    ...
	    Ext.getCmp('publicGrid').store.on('beforeload',
			function(store){ 
				if(Ext.getCmp('publicGrid').store.lastOptions != null){
				   Ext.getCmp('publicGrid').store.baseParams = {orgId:orgId,staffId:staffId,publicType:publicType,optype:optype,pagin:pagin,beginDate:beginDate,endDate:endDate};
				}
			}
	    )
    ...
    this.publicDel = function(){
        ...
        if (selGridItem != null && selGridItem.length >0){
	    	var objq = new Object();
	    	objq.optype = "D";
	        objq.messageId = selGridItem[0].data.messageId;
	        var tmpObj = invokeAction("/message/PublicMessageManageAction", objq);
	        oper.QryPublicGrid();
        }
    }
