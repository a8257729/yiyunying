...
public class OrgTmpJobTmpGridAction extends BaseActionAdapter {
	public Object doAction(HttpServletRequest request,
			HttpServletResponse response) {
		
		//获取参数
		int orgTmpId = Integer.parseInt((String)request.getParameter("orgTmpId"));
		String jsonStr = "";
		try {
			request.setCharacterEncoding("UTF-8");
			response.setContentType("text/html;charset=UTF-8");
			Map result = getOrgTmpDAO().getJobTmpGridByOrgTmp(orgTmpId);
