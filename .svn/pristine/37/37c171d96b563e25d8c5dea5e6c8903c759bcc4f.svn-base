...
/**
 * description :未配置规则列表查询
 * @author FL
 *
 */
public class QryOutSchRlsAction implements BaseAction {
	...
	public Object doAction(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			Map retMap = getScheduleMngDAO().getOutScheduleRules(paramMap,start,limit);
			...
			System.out.println("QryOutSchRlsAction jsonStr:----->: " + jsonStr);
