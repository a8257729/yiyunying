...
public class SessionManager implements ConfigDefs {
	 ...
	 */
	public Session createSession(Event anEvent,HttpSession httpSession) throws PushletException {
		//根据登录人员的staffId来创建session如果没有携带staffId则不创建session
		String staffId = anEvent.getField("staffId");
		Session retSession =  Session.create(staffId);
		System.out.println("mysession1 "+httpSession);
		retSession.setHttpSession(httpSession);
		try
		{
			UserTransaction userTransaction = TransactionHelper.getTransaction();
			userTransaction.begin();
			Map map = new HashMap();
			map.put("staffId", staffId);
			map.put("stateId", new Integer(1));
			map.put("onlineDate", new Date());
			Long imStaffOnlineId = getImStaffOnlineDAO().insert(map);
			retSession.getHttpSession().setAttribute("imStaffOnlineId", imStaffOnlineId);
