...
public class SelInterfHandler extends AbstractHandler {
	...
	protected void processHandle(Map paramMap) throws Exception {
		...
		try{
			...
			//} else */
				if("queryWorkOrderForEBiz".equals(methodStr)) {		//代办工单查询接口
				//测试数据
				/*StringBuilder responseXmlStrx = new StringBuilder("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>");
				responseXmlStrx.append("<Data>");
			...
			//System.out.println("返回 JSON是：" + newstr);
			...
			if (null == contentData) {
				...
				if(null != contentObj) {
					...
					if(bean.has("TroubleOrderStatus")){
						...
						newObj.put("name7","工单类别: "+ (TroubleOrderStatus.equals("1")?"障碍单":"装,移单"));
					
						}else {								
							newObj.put("orderStatus", "-1");//-1则没有
							newObj.put("name7","");
					}
				...
				if (null != contentData && 0 != contentData.size()) {
					...
					for (int i = 0; i < contentData.size(); i++) {
						...
						if(bean.has("ObstructProdType")){
						   newObj.put("ObstructProdType", bean.optString("ObstructProdType", "XDSL"));//产品标识
						}
						*/
						arr.add(newObj);
