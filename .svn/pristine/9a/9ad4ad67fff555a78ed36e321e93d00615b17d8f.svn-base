...
public class WeChatSaServiceImpl extends BaseService implements WeChatSaService
{
	...
	public Result getPhoneNum(JSONObject json){
		...
		try
		{
			...
			Map map = dao.queryUserPhoneInf(json.getString("qrcode"));
			if (null != map && map.size() > 0)
			{
				String phone = (String) map.get("cust_phone");
				resultMap.put("cust_phone", phone);
	...
	public Result checkUserName(JSONObject json)
	{
		...
		try
		{
			...
			if (null != map && map.size() > 0)
			{
				...
				String	 phone = (String) map.get("cust_phone");
				cust_name = phone.substring(phone.length() - 4, phone.length());
			}
	...
	public Result queryInstallationReport(JSONObject json) {
		...
		try {
			...
			String ooid = null;
			try {
				ooid = dao.queryorderId(serviceNum);
				logger.info("查询到的ooid:"+ooid);
			} catch (Exception e) {
				e.printStackTrace();
			}
			String createDate= null;
			String acceptPower= null;
			String maxnetspeed = null;
			try {
				createDate = dao.createDate(ooid);
				logger.info("查询到的装机时间:"+createDate);
				acceptPower = dao.guangshuai(ooid);
				logger.info("查询到的光衰:"+acceptPower);
				maxnetspeed = dao.maxnetspeed(ooid,serviceNum);
				logger.info("查询到的最大速率:"+maxnetspeed);
			} catch (Exception e) {
				e.printStackTrace();
			}
			resultMap.put("ooid", ooid);
			resultMap.put("createDate", createDate);
			resultMap.put("acceptPower", acceptPower);
			resultMap.put("maxnetspeed", maxnetspeed);
			result = new Result();
			result.setResultData(resultMap);
			result.setResultCode(1000);
		} catch (Exception e) {
	...
	public JSONObject queryUserHouseInfoByCode(JSONObject json)
	{
		...
		try
		{
			...
			String respByKT ="";
			String respBySA ="";
			JSONObject jsonByKT=null;
			JSONObject jsonBySA=null;
			if (null != map && map.size() > 0)
			{
				...
				respByKT = dao.queryWorkOrderInfo(service_num);
				respBySA = dao.queryWorkOrderInfoSA(service_num);
				jsonByKT = JSONObject.fromObject(respByKT);
				jsonBySA = JSONObject.fromObject(respBySA);
				//记录日志service_num
			...
			jsonObject.put("jsonByKT",jsonByKT);
			jsonObject.put("jsonBySA",jsonBySA);
