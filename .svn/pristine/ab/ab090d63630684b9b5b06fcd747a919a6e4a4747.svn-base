 ...
 * @version $Id: Pushlet.java,v 1.22 2007/11/24 10:29:36 justb Exp $
 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (Enumeration e = request.getParameterNames(); e.hasMoreElements();) {
				...
				//对输入流转码
				String str = new String(request.getParameter(nextAttribute).getBytes("iso-8859-1"), "gbk");
				
				event.setField(nextAttribute, str);
			}
	...
	protected void doRequest(Event anEvent, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (eventType.startsWith(Protocol.E_JOIN)) {
				...
				//System.out.println("join+++ "+eventType);
				//判断创建sessionn时是否有staffId
				String staffId = anEvent.getField("staffId");
				//System.out.println("sessionId+++ "+staffId);
				if(staffId==null||staffId.trim().equals("")){
					return ;
				}
				System.out.println("httpsession :"+request.getSession());
				session = SessionManager.getInstance().createSession(anEvent,request.getSession());
