...
/**
 * description : 数据库列表查询Action
 * @author FL
 *
 */
public class QryDataBaseListAction implements BaseAction {
	...
	public Object doAction(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			Map retMap = getDataBaseMngDAO().getDataBaseList(paramMap,start,limit);
			...
			System.out.println("QryDataBaseListAction jsonStr:----->: " + jsonStr);
