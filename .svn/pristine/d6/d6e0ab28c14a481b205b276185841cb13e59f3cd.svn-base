...
public class EomsOrderDocDAOMSSQL
    extends AbstractDAOImpl
    implements EomsOrderDocDAO {

    /**
     * 空构造方法
     */
    public EomsOrderDocDAOMSSQL() {
    }
     ...
     */
    public EomsOrderDocDAOMSSQL(String dsName, boolean debug) {
        super(dsName, debug);
    }
    ...
    SQLException, RequiredException, TooLongException, UOSException {
	    if (!ValidateHelper.validateNotNull(eomsOrderDocDto.getId())) {
	        eomsOrderDocDto.setId(DAOHelper.getPKFromDB("eoms_order_doc",
	            9));
	    }
    ...
    public void setDocumentContent(long fileId,
		...
		//update data
		String sqlStr =
		"UPDATE EOMS_ORDER_DOC SET DOCUMENT_CONTENT=? WHERE ID=? ";
		...
		try {
			conn = getConnection();
			ps = conn.prepareStatement(sqlStr);
			DAOUtils.setImage(ps,1, fileBytes);
			ps.setLong(2, fileId);
			ps.executeUpdate();
