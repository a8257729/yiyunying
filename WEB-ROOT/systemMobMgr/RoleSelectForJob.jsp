...
Ext.onReady(function(){
    ...
    var confRoleTree = new Ext.tree.TreePanel({
        ...
        autoScroll:true,
        loader: new Ext.tree.TreeLoader({
        	dataUrl:'/MOBILE/ExtServlet?actionName=system/OrgRoleConfTreAction&areaId=' + areaId,
        	baseParams: {jobId:jobId,specialJobId:specialJobId,_jobId:_jobId,methodName:methodName}
        }),
        enableDD:true,
        containerScroll: true,
        border: true,
    ...
    orgTree.on('click', function(node){
	    ...
	    areaId = node.attributes.id ;
	  	...
	  	confRoleTree.getLoader().dataUrl = '/MOBILE/ExtServlet?actionName=system/OrgRoleConfTreAction&areaId='+areaId;
    ...
    Ext.get('comitBtn').on('click',function(e){
		...
		callRemoteFunction("com.zterc.uos.oaas.service.jobprivmanager.JobPrivilegeManagerWeb", "updateJobRoles", _jobId, selectedRoles, canGrant);
		var paramObj = new Object();
    	paramObj._defaultJobId = _jobId;    	
    	paramObj.priv = ['-1'];
    	paramObj.roleIds = selectedRoles;
    	paramObj.strpriv = '-1';
		paramObj.type = "rolePrivAdd";
		invokeAction("/mobsystem/InsertPrivAction", paramObj);
