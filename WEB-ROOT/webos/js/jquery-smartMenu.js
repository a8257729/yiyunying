				...
				$.each(dataMenu, function(i, arr) {
					...
					if ($.isArray(arr)) {
						$.each(arr, function(j, obj) {
							var text = obj.text, imgurl = obj.imgurl, htmlMenuLi = "", strTitle = "", rand = Math.random().toString().replace(".", "");
							...
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									htmlMenuLi = '<li class="'+ clKey +'li" data-hover="true">' + htmlCreateMenu(obj.data) ;
										if (imgurl) {
												htmlMenuLi = htmlMenuLi + '<img class="'+ clKey +'img"'+ ' alt="'+ text +'" src="'+ imgurl +'">' +
													'<a href="javascript:" class="'+ clKey +'img_a"'+ strTitle +' data-key="'+ rand +'"><i class="'+ clKey +'triangle"></i>'+ text +'</a>' + '</li>';
											}else{
												htmlMenuLi = htmlMenuLi + '<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'"><i class="'+ clKey+'triangle"></i>'+ text +'</a>' + '</li>';
											}	
								} else {
									htmlMenuLi = '<li class="'+ clKey +'li">' ;
										if (imgurl) {
												htmlMenuLi = htmlMenuLi + '<img class="'+ clKey +'img"'+ ' alt="'+ text +'" src="'+ imgurl +'">' +
												'<a href="javascript:" class="'+ clKey +'img_a"'+ strTitle +' data-key="'+ rand +'">'+ text +'</a>' + '</li>';
											}else{
												htmlMenuLi = htmlMenuLi + '<a href="javascript:" class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'">'+ text +'</a>' + '</li>';
										}
								}
		...
		$(this).each(function() {
			this.oncontextmenu = function(e) {
				...
				if (jqueryMenu) {
					//判断
					var mTop = 0;
					if((e.clientY + st + params.offsetY  + jqueryMenu.height())> $(document).height()) {
						mTop = e.clientY + st + params.offsetY - jqueryMenu.height();
					} else {
						mTop = e.clientY + st + params.offsetY;
					}
					jqueryMenu.css({
						display: "block",
