...
public class DbOperDecorate {
	...
	public int[] dynamicInsertMapBatch(List paramMapList, String tableName,
		...
		if (patternStr.indexOf("@@SEQ") != -1) {
			...
			DbOperSupportFactory.getFactory().getDbOperSupport()
					.preparedInsertBatch(paramMapList, tableName, keyName,
							dbOper);
			...
			_return = dbOper.dynamicInsertMapBatch(paramMapList, tableName,
					patternStrArr, sql);

			DbOperSupportFactory.getFactory().getDbOperSupport()
					.lastedInsetBatch(paramMapList, tableName, keyName, dbOper);
