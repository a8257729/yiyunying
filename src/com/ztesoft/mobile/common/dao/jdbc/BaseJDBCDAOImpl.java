package com.ztesoft.mobile.common.dao.jdbc;
...
import com.ztesoft.mobile.common.db.DbOperDecorate;
...
public class BaseJDBCDAOImpl{
	...
	//add by feng.yang 2011.4.21
	protected Map populateQueryByMapNoAs(StringBuffer sqlStr, int startIndex,
		...
		try {
			...
			return dbOper.populateQueryByMapNoAs(sqlStr, startIndex, stepSize);
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new DataAccessException(ex.getMessage(), ex);
		} finally {
...
//end	
	...
	protected List populateQueryByMapNoAs(StringBuffer sqlStr) throws DataAccessException {
		Connection conn = null;
		DbOper dbOper = null;
		try {
			conn = getConnection();
			dbOper = new DbOper(conn);
			return dbOper.populateQueryByMapNoAs(sqlStr);
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new DataAccessException(ex.getMessage(), ex);
		} finally {
	...
	protected List dynamicQueryListByMap(String sqlStr) throws DataAccessException {
		Connection conn = null;
		try {
			conn = getConnection();
			DbOper dbOper = new DbOper(conn);
			return dbOper.dynamicSelForMapListByNew(sqlStr);
		} catch (Exception ex) {
			ex.printStackTrace();
			throw new DataAccessException(ex.getMessage(), ex);
		} finally {
