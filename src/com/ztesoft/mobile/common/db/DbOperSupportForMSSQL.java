...
import com.ztesoft.mobile.common.helper.*;
    ...
    public void setPrepValue(String[] patternStrArr, PreparedStatement ps,
        ...
        try {
            ...
            for (int j = 0; j < patternStrArr.length; j++) {
        		...
        		//新加类型 03年5月3日******************************************************************
				if ("clob".equalsIgnoreCase(_info.getColumnType())
						|| "blob".equalsIgnoreCase(_info.getColumnType())) {
					k = 6;
					errorKey = mapKey;
					//保存clob数据类型的时候 可以把它当作string保存，也可以用流的形式来保存
					if (ValidateHelper.validateNotNull(mapValue)) {
						mapValue = String.valueOf(mapValue);
						DbOper.setPrepStatementParam(loop++, ps, StringHelper
								.stringValueOf(mapValue));
						//这个是以流的形式保存clob数据
//						byte[] bytes = mapValue.toString().getBytes();
//						ByteArrayInputStream baisss = new ByteArrayInputStream(
//								bytes);
//						InputStreamReader bais = new InputStreamReader(baisss);
//						DbOper.setPrepStatementParam(loop++, ps, bais, bytes);
					}
				}
				//***************************************************************************************
