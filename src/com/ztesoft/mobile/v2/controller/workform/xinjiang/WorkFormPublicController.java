...
//	        Long areaId = json.optLong(JobInfo.AREA_ID_NODE);
		 ...
		 *
		 */
		@RequestMapping(value = {"/client/xj/web/tree/fault/reason/query1"})
		public @ResponseBody Result qryFaultHandTree(@RequestBody Map<String,Object> data)
		{
			logger.debug(" Call qryFaultHandTree method ");

			JSONObject json = new JSONObject();
			json.put("JobId",(String)data.get("JobId"));
//	        Long jobId = json.optLong(JobInfo.JOB_ID_NODE);
//	        Long areaId = json.optLong(JobInfo.AREA_ID_NODE);
			Result result = getWorkOrderService().qryHandTree(json);
			return result;
		}
	 ...
	 *
	 */
	@RequestMapping(value = {"/client/xj/web/staff/query/area"})
	public @ResponseBody Result qryStaffIdArea(@RequestBody Map<String,Object> data)
	{
		logger.debug(" Call qryFaultHandTree method ");
...
//	        Long jobId = json.optLong(JobInfo.JOB_ID_NODE);
//	        Long areaId = json.optLong(JobInfo.AREA_ID_NODE);
		Result result = getWorkOrderService().qryStaffIdArea(json);
		return result;
	}
