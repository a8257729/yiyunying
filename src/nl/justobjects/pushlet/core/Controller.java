...
import com.ztesoft.eoms.im.MsgDispatchService;
...
public class Controller implements Protocol, ConfigDefs {
	...
	public void doCommand(Command aCommand) {
		try {
			...
			if (eventType.endsWith(Protocol.E_LISTEN) ||
				...
				//System.out.println("sysout command "+aCommand.toString());
	...
	protected void doPublish(Command aCommand) {
		Event responseEvent = null;
		MsgDispatchService msgDispatchService = new MsgDispatchService();
		try {
			...
			if (subject == null) {
				...
				if (subject.equals("/user/chat,hb")){ 
					
					msgDispatchService.dispatchMsg(aCommand, session);
					...
					int groupId = Integer.parseInt(subject.split("/")[2]);
					msgDispatchService.dispatchGroupMsg(aCommand, session, groupId);
				}else {
					if (to != null) {
						Dispatcher.getInstance().unicast(aCommand.reqEvent, to);
					} else {
